...
class CNN_FTLSTM(nn.Module):
    def __init__(self,in_channels, out_channels, kernel_size_cnn, 
        ...
        self.classification_raw1=nn.Linear(2*hidden_dim,self.num_labels).to(self.device)
    ...
    def forward(self,input_lstm,input1,input2,target,seq_length,train=True):
        ...
        out1 = self.classification_raw1(torch.cat([out1,out2],dim=1))
        ...
        losses_batch_raw=F.cross_entropy(out1,torch.max(target,1)[1])
