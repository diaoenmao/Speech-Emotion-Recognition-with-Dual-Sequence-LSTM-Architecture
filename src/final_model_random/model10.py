...
class FTLSTMCell(nn.Module):
    def __init__(self,  inputx_dim,inputy_dim,hidden_dim, max_length, device,dropout=0.5):
        ...
        self.batchhT = nn.SeparatedBatchNorm1d(num_features=self.hidden_dim,max_length=max_length)
    ...
    def __init__(self,time,inputx_dim,inputy_dim,hidden_dim,num_layers_ftlstm,device):
        ...
        for i in range(num_layers_ftlstm):
            ...
            if i==num_layers_ftlstm-1:
                cell = FTLSTMCell(hidden_dim,inputy_dim,hidden_dim,self.time,self.device)
            else:
                cell = FTLSTMCell(inputx_dim,inputy_dim,hidden_dim,self.time,self.device)
            setattr(self, name, cell)
            self._all_layers.append(cell)
