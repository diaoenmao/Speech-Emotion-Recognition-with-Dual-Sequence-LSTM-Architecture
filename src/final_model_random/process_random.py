...
class IEMOCAP(Dataset):
    def __init__(self, fold, train=True):
        if train:
            f = open('/scratch/speech/final_dataset_random/EMO39_mel_spectrogram_train_{}.pkl'.format(fold), 'rb')
        else:
            f = open('/scratch/speech/final_dataset_random/EMO39_mel_spectrogram_test_{}.pkl'.format(fold), 'rb')
        data = pickle.load(f)
