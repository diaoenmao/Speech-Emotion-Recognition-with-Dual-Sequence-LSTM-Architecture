...
def train_model(args):
    if args.model=="model9": from model9 import CNN_FTLSTM,CNN_FTLSTM_no_hand, LSTM_Audio
    ...
    for fold in range(5):
        for mo in ["CNN_FTLSTM"]:
            ...
            if mo=="CNN_FTLSTM":
                model = CNN_FTLSTM(input_channels, out_channels, kernel_size_cnn,stride_size_cnn, kernel_size_pool, stride_size_pool,nfft,hidden_dim,num_layers_ftlstm,weight,device)
            print("============================= {} starts =======================================".format(mo))
            ...
            path="mo:{};model:{};batch_size:{};out_channels:{};kernel_size_cnn:{};weight:{};lr:{}".format(mo,args.model,args.batch_size,out_channels,kernel_size_cnn,weight,args.lr)
            ...
            for epoch in range(epoch_num):  # again, normally you would NOT do 300 epochs, it is toy data
                ...
                if class_accuracy_test==max(class_acc) and accuracy_test==max(test_acc):
                    torch.save(model.module.state_dict(),"/scratch/speech/models/final_classification_random/{}_pretrained.pt".format(mo))
