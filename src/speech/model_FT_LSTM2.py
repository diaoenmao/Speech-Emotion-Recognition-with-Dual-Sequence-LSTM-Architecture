...
class FTLSTMCell(nn.Module):
    def __init__(self,  inputx_dim,inputy_dim,hidden_dim, dropout=0):
        ...
        self.WTx=nn.Parameter(torch.Tensor(self.inputx_dim,3*self.hidden_dim))
        self.WFx=nn.Parameter(torch.Tensor(self.inputx_dim,3*self.hidden_dim))
        self.WTy=nn.Parameter(torch.Tensor(self.inputy_dim,3*self.hidden_dim))
        self.WFy=nn.Parameter(torch.Tensor(self.inputy_dim,3*self.hidden_dim))
        self.WTh=nn.Parameter(torch.Tensor(self.hidden_dim,3*self.hidden_dim))
        self.WFh=nn.Parameter(torch.Tensor(self.hidden_dim,3*self.hidden_dim))
        self.biasT=nn.Parameter(torch.Tensor(3*self.hidden_dim))
        self.biasF=nn.Parameter(torch.Tensor(3*self.hidden_dim))

        self.WTCx=nn.Parameter(torch.Tensor(self.inputx_dim,self.hidden_dim))
        self.WFCy=nn.Parameter(torch.Tensor(self.inputy_dim,self.hidden_dim))
        self.WTCh=nn.Parameter(torch.Tensor(self.hidden_dim,self.hidden_dim))
    ...
    def forward(self, x,y,hT,hF,CT,CF):
        sz=hT.shape[1]
        gatesT=x@self.WTx + y@self.WTy + hT@self.WTh + self.biasT
        gatesF=x@self.WFx + y@self.WFy + hF@self.WFh + self.biasF
        C_T=x@self.WTCx + hT@self.WTCh + self.biasCT
        C_F=y@self.WFCy + hF@self.WFCh + self.biasCF
        fT,iT,oT=(torch.sigmoid(gatesT[:,:sz]),
                torch.sigmoid(gatesT[:,sz:2*sz]),
                torch.sigmoid(gatesT[:,2*sz:]))
        fF,iF,oF=(torch.sigmoid(gatesF[:,:sz]),
                torch.sigmoid(gatesF[:,sz:2*sz]),
                torch.sigmoid(gatesF[:,2*sz:]))
        CT=fT*CT+iT*C_T
        CF=fF*CF+iF*C_F
        hT=oT*torch.tanh(CT)
        hF=oF*torch.tanh(CF)
