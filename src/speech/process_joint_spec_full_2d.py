...
class IEMOCAP(Dataset):
    def __init__(self, name, nfft, train=True):
        if train:
            pickle_in = open('/scratch/speech/hand_raw_dataset/EMO39_'+name+'_spectrogram_augmented_nfft{}_train.pkl'.format(nfft), 'rb')
        else:
            pickle_in=open('/scratch/speech/hand_raw_dataset/EMO39_'+name+'_spectrogram_augmented_nfft{}_test.pkl'.format(nfft), 'rb')
    ...
    def __getitem__(self, index):
        sample = {'input_lstm': torch.from_numpy(self.input_lstm[index]).float(),
                  ...
                  'input': torch.squeeze(F.interpolate(torch.unsqueeze(torch.from_numpy(10*np.log10(self.input[index])).float(), dim=0), size=140, mode='linear'), dim=0),
