...
class IEMOCAP(Dataset):
    ...
    def __getitem__(self, index):
        input1 = torch.squeeze(F.interpolate(torch.unsqueeze(torch.from_numpy(10*np.log10(self.input1[index])).float(), dim=0), size=256, mode='nearest'), dim=0)
        input2 = torch.squeeze(F.interpolate(torch.unsqueeze(torch.from_numpy(10*np.log10(self.input2[index])).float(), dim=0), size=256, mode='nearest'), dim=0)
