...
for i in range(10):
    ...
    model = ATT(num_features=39, hidden_dim=300, num_layers=2, dropout_rate=0.6,
                     num_labels=4, batch_size=128,bidirectional=True)
    model = model.cuda()
    diri="/scratch/speech/models/classification/deep_att_classifier_epoch_"+str((i+1)*10)+".pt"
