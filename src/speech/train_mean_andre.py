...
model = Mean_Pool_2(num_features=39, hidden_dim=300, num_layers=2, dropout_rate=0.2, num_labels=4, batch_size=128,bidirectional=True)
model.cuda()
...
#        print("==============================Batch " + str(j) + "=============================================")
    ...
    if (epoch + 1) % 10 == 0:
        torch.save(model.state_dict(), '/scratch/speech/models/classification/deep_mean_pool_epoch_' + str(epoch+1) + '.pt')
        print('/scratch/speech/models/classification/deep_mean_pool_epoch_' + str(epoch+1) + '.pt')

torch.save(model.state_dict(), '/scratch/speech/models/classification/deep_mean_pool_classifier.pt')
